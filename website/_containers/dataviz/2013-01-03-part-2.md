...
Notice how we assign `data_file` to our parse function, and the parameters we feed through our parse function are `MY_FILE` and a comma-delimiter. Because we know the parse function returns `parsed_data`, we can expect that `data_file` will be that exact return value.
...
Here, `data_list` takes each key of `counter` to grab the value associated with each day.  Because we manually write out each `counter` key, we enforce the order that we want. **Note:** a dictionary does _not_ preserve order, but a list does; this is why we’re electing to manually key into each value of a dictionary to make a list of each value.
...
When you’re done marveling at your work, close the image and get back to your terminal.
...
Now we assign our x-ticks (should be familiar from `visualize_days()`):
...
Now we can then plot the y-axis values as a bar graph:
